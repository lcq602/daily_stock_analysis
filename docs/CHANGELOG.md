...
- [改进] DeepSeek 兼容与定时时区配置增强：当 `OPENAI_BASE_URL` 指向 DeepSeek 且未显式配置 `DEEPSEEK_API_KEY(S)` 时，会自动复用 `OPENAI_API_KEY(S)`；新增 `SCHEDULE_TIMEZONE`（IANA 时区，如 `Asia/Shanghai`）用于按指定时区执行每日调度。
